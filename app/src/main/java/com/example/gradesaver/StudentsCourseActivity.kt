...
import com.example.gradesaver.database.entities.Course
import com.example.gradesaver.database.entities.User
import com.example.gradesaver.repos.ActivityRepository
...
class StudentsCourseActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        repository = ActivityRepository(database.appDao(), notificationScheduler)
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            val userEnrolledCourses = db.getEnrollmentsByStudent(user.userId).mapNotNull { enrollment ->
                ...
                courses.forEach { course ->
                    subscribeToActivityUpdates(course.courseId)  // Add this line
                }
            }
    ...
    private fun subscribeToActivityUpdates(courseId: Int) {
        val db = AppDatabase.getInstance(applicationContext)
        db.appDao().fetchActivitiesByCourseAsLiveData(courseId).observe(this, Observer { activities ->
            // Update UI or react to changes
            adapter.updateActivitiesForCourse(courseId, activities)
        })
    }
        ...
        lifecycleScope.launch {
            ...
            enrollments.forEach { enrollment ->
                db.appDao().getCourseById(enrollment.courseId)?.let { course ->
                    courses.add(course)
                    details[course] = db.appDao().getActivitiesByCourse(course.courseId)
                    subscribeToActivityUpdates(course.courseId)  // Ensure LiveData is observed for each course
                }
            }
            runOnUiThread {
                adapter.updateData(courses, details)
